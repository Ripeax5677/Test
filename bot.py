...
Config = {
    "Bot Name": "Spinstar Casino",  # will be in all 
    "Bot Icon": "https://cdn.discordapp.com/attachments/1338957502733094984/1339044816079687710/IMG_0909.webp?ex=67b13eaa&is=67afed2a&hm=1731f96b12080d88a7b9367061965a0a85c9afe4deed0ba6607c6b895bd8b6f8&",  # will be in all embeds
    ...
    "Logs": 1338957505367380091,  # log channel
    "Coinflip": {  # Config for coinflip
        "1v1": "1338957488350826549",  # Channel That Coinflips Be In
        "House": 5  # House Edge (%)
    },
    "Rains": {  # Config for rains
        "Channel": "1338957490779328564",  # Set to the id the channel rains will be in
    },
    "Status": {
        "Message": "Offline",
    },
    "AdminCommands": {
        "UserID": ["1327221904079196170", "1327221904079196170", "1327221904079196170", "1327221904079196170", "1327221904079196170", "1327221904079196170", "1327221904079196170"],
        "OwnerID":
["1327221904079196170"],
    },
    "AutoDeposits": {
        "Webhook": "https://discord.com/api/webhooks/1339045415798050836/gCwFVoHjMksJ8WTdZU1vM5fyY70bugsV8iBOkdjMTBaQCnQzxpByMYmHhvw6QDBY7Leh"  # auto deposits or /confirmdeposit
    },
    "Withdraws": {
        "Webhook": "https://discord.com/api/webhooks/1339045617418113046/OXQTmBQtL6MUsDZOY2mMZ41yl4d9SQNDEqxfVSIG3Y3afDmYsTSopuV7WIIc0CyGlyB2",
    },
    "Affiliates": {
        "Webhook": "https://discord.com/api/webhooks/1339045791028740157/aSX3mqN31Y4TfxnXSanQWxkk3uzvuZGxhcsBpApxr_33xUthFSfJ-S2CFLB1qlsev9k-",  # webhook for when someone gets affiliated
    },
    "Tips": {
        "Webhook": "https://discord.com/api/webhooks/1339045971778338837/lF0u32C5zG0_HI9xU1nUGQoRnCD_fY0_4jHoF0QksADY-kbCCUsXachmnSHitNp9lWXD",  # webhook for tips
    },
    "Promocodes": {
        "Webhook": "https://discord.com/api/webhooks/1339046133682671718/5CLD43ne-qGBm2wEmyrpDHDsDmJXLzzLuM5tK94g20SHCGW4aYIexI0fgf2wcSmfiTSQ",  # webhook for promocodes
        "RoleID": "1338957444759687320",  # for ping
    },
    "Upgrader": {  # Config for upgrader
        "House": 0.9  # house edge (winnings*house)
    },
    "Rakeback": 1,  # Rakeback %
    "Username": "",  # The Username Of The Account Running The Bot
    "DiscordBotToken": "MTMzOTA0NjQwNTgwMDU5MTQzMA.G-BWUS.Ki2oSngFLYmcFWEGqMP7GpzFv44vKFp_GjsxNs",  # The token of the discord bot
    "AddGemsWebhook": "https://discord.com/api/webhooks/1336589396706594827/jLQtDec7JHSgf9V196mqiDqQe56q-GqIo0WZfptuccBh3XPOP7MWSJiebZ1-F2ZdB4LW",
    "SetGemsWebhook": "https://discord.com/api/webhooks/1336589405280010261/FmbnbuoFjbRdLZj6c_px9-66F86Kx3YXnjrayEw-tVmW7szm0IQ0E5OnL3p6nlgQGovE"# New private channel webhook for /addgems
}
...
api_url = "http://node1.adky.net:1532/api/pet"
...
def get_timestamp(last_updated):
    try:
        ...
        match = re.search(r"(\d+)\s*(hour|hours|day|days|week|weeks)", last_updated.lower())
        if not match:
        ...
        if "week" in unit:
            past_date = datetime.utcnow() - timedelta(weeks=num)
        elif "day" in unit:
            past_date = datetime.utcnow() - timedelta(days=num)
        elif "hour" in unit:
            past_date = datetime.utcnow() - timedelta(hours=num)
        else:
...
async def pet_value(interaction: discord.Interaction, pet_name: str):
    ...
    if pet_details is None:
        await interaction.response.send_message("Pet not found. Please check the name and try again.", ephemeral=True)
        return
    ...
    embed.add_field(name="Withdraw Price", value=pet_details["value"], inline=True)
    embed.add_field(name="Last Updated", value=f"<t:{timestamp}:R>", inline=True)
    
    embed.set_thumbnail(url=pet_details["image_url"])
    embed.set_footer(text=f"{Config['Bot Name']} - Credits to Cosmic Values", icon_url=Config['Bot Icon'])
